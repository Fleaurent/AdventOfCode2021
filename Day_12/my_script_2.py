...
class Tree():
    ...
    def build_tree(self, node: 'TreeNode') -> None:
        ...
        for neighbor_node in self.network.network[node.name]:
            parent_nodes = node.get_parent_nodes_lowercase()
            # 1. big caves can be visited any number of times
            # 2. a single small cave can ve visited at most twice
            # 3. -> the remaining small caves can be visited at most once
            # 4. start and end have to be visited exactly once
            if neighbor_node not in parent_nodes:
                node.add_leaf(neighbor_node)
                self.build_tree(node.leafs[-1])
            elif neighbor_node == 'start':
                continue
            else:
                # check if one small cave has been visited twice already
                if not self.visited_twice(parent_nodes):
                    node.add_leaf(neighbor_node)
                    self.build_tree(node.leafs[-1])

    def visited_twice(self, nodes: list[str]) -> bool:
        return any(nodes.count(node) > 1 for node in nodes)
    
    def parse_tree_paths(self) -> list[list[str]]:
        return self._parse_tree_paths(self.tree)
    
    def _parse_tree_paths(self, node: 'TreeNode') -> list[list[str]]:
        if node.leafs:
